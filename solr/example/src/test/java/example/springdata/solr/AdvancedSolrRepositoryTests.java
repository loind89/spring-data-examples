...
public class AdvancedSolrRepositoryTests {
	...
	static class Config extends SolrTestConfiguration {
		...
		/*
		 * (non-Javadoc)
		 * @see example.springdata.solr.SolrTestConfiguration#doInitTestData(org.springframework.data.repository.CrudRepository)
		 */
		@Override
	...
	public void influcenceScoreWithFunctions() {
		...
		operations.queryForPage(new SimpleQuery(where(exists("popularity"))).//
				addProjectionOnFields("*", "score"), Product.class).//
				forEach(System.out::println);
	}
}
